...
The final model is
$$outstate=21174.4471-0.5810H(15622-expend)-28.9147H(83-grad\_rate)-0.8783H(5318-room\_board)$$ 
$$-1.2010H(f\_undergrad-1362)+0.3331H(apps-7875)-0.6461H(7875-apps)$$
$$+0.8936H(f\_undergrad-4772)+1.0816H(1345-personal)-90.1638H(15-perc\_alumni)$$
$$+24.7242H(terminal-69)-0.8752H(1577-accept)$$ The partial dependence plot shows a linear increasing of `outstate` as `perc_alumni` increases with a cut point at 15 pct. alumni who donate, holding other variables constant. Also, the plot of `perc_alumni` and `apps` is also shown to visualize their impact on the outcome `outstate`. The test error is `r mars.test.error`.
...
The GAM model includes all the predictors. There is no bivariate function in the model, which means no interaction between predictors. The plot shows the 16 predictors (from left to the right, from top to the bottom) `perc_alumni`, `terminal`, `books`, `ph_d`, `top10perc`, `grad_rate`, `top25perc`, `s_f_ratio`, `personal`, `p_undergrad`, `enroll`, `room_board`, `accept`, `f_undergrad`, `apps`, `expend`. We can see some from the plots that some predictors show obvious nonlinear function terms such as `ph_d`, `accept`, `f_undergrad`, `apps`, `expend`. Also, the credible intervals are generally narrow on each plot which means the uncertainty is well controlled at each predictor, except at `accept` or `f_undergrad` where the intervals both have a large range as the respective predictors increase. The test error is `r gam.test.error`.
